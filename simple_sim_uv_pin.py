...
class SimpleSimulationRigCreator(object):
    ...
    def _create_locators(self):
        ...
        locators = []
        for selection in self.selections:
            name = "{0}_{1}_locator".format(self.name, selection.short_name)
    ...
    def _connect_rig_components(self):
        ...
        self._animate_dynamic_constraints()
        # create and connect all the attributes to the user controller
        self._connect_simulation_controller()
        # connect blend locators to curve
        self._connect_locators_to_dynamic_curve()
    ...
    def _connect_locators_to_dynamic_curve(self):
        """connect locators to dynamic curve using the UV Pin node"""
        for number, locator in enumerate(self.blend_locators.nodes):
            self.dynamic_curve.uv_pin(locator, number)

    def _connect_selection_to_rig(self):
        """Connect the selection to the rig"""
        for selection, locator in zip(self.selections, self.blend_locators.nodes):
            constraint_creation_utils.create_parent_constraint(
                parent=locator.long_name,
                child=selection.long_name
            )

    def _animate_dynamic_constraints(self):
    ...
    def _delete_rig(self):
        ...
        rig_setup = [self.sim_controller, self.curve, self.blend_locators,
...
class MayaNode(object):
    ...
    def attach_to_node(self, attachment_attribute_name):
        """attach this node to another node with a message attribute. Used to connect to main group of simple simulation
        :param str attachment_attribute_name: name of attribute to attach this node to"""
        return
