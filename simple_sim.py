...
class SimpleSimulationRigCreator(object):
    ...
    def _connect_bones_to_dynamic_curve(self):
        ...
        self.ik_handle = MayaNode(ik_handle[0])
...
class Nucleus(MayaNode):
    ...
    def add_colliders(self, maya_node_list):
        ...
        for maya_node in maya_node_list:
            n_rigid_node = cmds.createNode("nRigid")
            cmds.connectAttr("time1.outTime", "{0}.currentTime".format(n_rigid_node))
            cmds.connectAttr("{0}.outTime".format(maya_node.shape), "{0}.inputMesh".format(n_rigid_node))
            cmds.connectAttr(self.startFrame, "{0}.startFrame".format(n_rigid_node))
            input_passive_attribute = "{0}.inputPassive[0]".format(self.long_name)
            input_passive_start_attribute = "{0}.inputPassiveStart[0]".format(self.long_name)
            # TODO: make this into a separate function to grab indexed attributes
            for i in range(100):
                connected = cmds.connectionInfo("{0}.inputPassive[{1}]".format(self.long_name, i), sourceFromDestination=True)
                if not connected:
                    input_passive_attribute = "{0}.inputPassive[{1}]".format(self.long_name, i)
                    input_passive_start_attribute = "{0}.inputPassiveStart[{1}]".format(self.long_name, i)
                    break
            cmds.connectAttr("{0}.currentState".format(n_rigid_node), input_passive_attribute)
            cmds.connectAttr("{0}.startState".format(n_rigid_node), input_passive_start_attribute)
            
            cmds.setAttr("{0}.quadSplit".format(maya_node.shape), 0)
